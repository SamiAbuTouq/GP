...
export class MailService {
  ...
  private async sendMailWithRetries(
    ...
    if (!config) {
      this.logger.warn(
        `${contextLabel} email skipped: SMTP configuration is incomplete.`,
      );
      return;
    }
    ...
    for (const candidate of candidates) {
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          if (attempt > 1) {
            this.logger.log(
              `${contextLabel} email succeeded on retry ${attempt}.`,
            );
          }
          return;
        } catch (error) {
      ...
      <div style="font-family: Arial, sans-serif; background-color: #f4f5f7; padding: 40px 20px;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #e2e8f0;">
          <div style="background-color: #1a365d; padding: 30px;">
            <h1 style="color: #ffffff; margin: 0; font-size: 22px;">Smart University Timetabling System</h1>
    ...
    const coursesHtmlBlock =
      params.courses.length > 0
        ? `<ul style="margin: 8px 0 0 18px; padding: 0; color: #475569; font-size: 14px; line-height: 1.55;">
            ${params.courses
              .map(
                (c) => `<li style="margin: 4px 0;">${this.escapeHtml(c)}</li>`,
              )
              .join("")}
          </ul>`
