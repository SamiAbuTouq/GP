...
let TimetablesService = class TimetablesService {
    ...
    async getTimetableConflictSummary(timetableId) {
    ...
    async ensureHardConflictsAcknowledged(timetableId, acknowledgedHardConflicts) {
    ...
    async publishDraftTimetable(timetableId, params, publisher) {
        ...
        if (isScenarioSandbox) {
            throw new common_1.BadRequestException('Scenario-result timetables cannot be promoted to a published timetable. Run the scenario against an optimizer draft or published schedule; only those drafts may be published.');
        }
        await this.ensureHardConflictsAcknowledged(timetableId, params.acknowledgedHardConflicts);
        ...
        for (const uid of lecturerIds) {
            void this.notifications
                ...
                preferenceKey: isRevision
                    ? notification_prefs_1.LECTURER_NOTIFICATION_PREF_KEYS.SCHEDULE_REVISED
                    : notification_prefs_1.LECTURER_NOTIFICATION_PREF_KEYS.SCHEDULE_PUBLISHED,
            })
                .catch(() => { });
        }
    ...
    async notifyPreferenceViolationsForTimetable(timetableId) {
        ...
        for (const e of entries) {
            if (e.user_id == null)
                continue;
            if (!badSet.has(`${e.user_id}:${e.slot_id}`))
                continue;
            ...
            const label = slot
                ...
                : `slot ${e.slot_id}`;
            void this.notifications
                .createForUser(e.user_id, 'Preference Not Honored', `You were assigned ${label}, which you marked as unavailable. If this is unexpected, contact the timetabling office.`, { preferenceKey: notification_prefs_1.LECTURER_NOTIFICATION_PREF_KEYS.PREFERENCE_NOT_HONORED })
                .catch(() => { });
        }
    }
    ...
    async buildSchedulePayload(timetableId) {
        const timetable = await this.prisma.timetable.findUnique({
            where: { timetable_id: timetableId },
            include: {
                semester: true,
        ...
        for (const e of entries) {
            ...
            if (!usedRoomSlots.has(room))
                usedRoomSlots.set(room, new Set());
        ...
        const usedSlots = new Set(entries
            ...
            .map((e) => `${e.room.room_id}|${e.slot_id}`)).size;
        const roomUtilization = timetable.timetable_metrics != null
        ...
        return {
            ...
            metadata: {
                ...
                iterations: null,
                wolves: null,
                best_fitness: timetable.timetable_metrics != null
    ...
    async replaceScheduleFromPayload(timetableId, scheduleRaw) {
        ...
        const virtualRoomId = rooms.find((r) => r.room_type === 3)?.room_id ??
            rooms[0]?.room_id ??
            null;
        ...
        for (const raw of scheduleRaw) {
            ...
            if (!Number.isFinite(slotId) || slotId <= 0) {
                throw new common_1.BadRequestException(`Invalid timeslot id "${timeslot}".`);
            }
            const courseId = courseByCode.get(courseCode.toLowerCase());
            if (courseId == null) {
                throw new common_1.BadRequestException(`Unknown course code "${courseCode}".`);
            }
            ...
            if (needsRoom) {
                ...
                if (mappedRoomId == null) {
                    throw new common_1.BadRequestException(`Unknown room "${roomName}".`);
                }
                roomId = mappedRoomId;
            }
            rowsToCreate.push({
