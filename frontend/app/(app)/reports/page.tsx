...
export default function ReportsPage() {
  ...
  // Get available formats for the selected report
  const getAvailableFormats = () => {
    if (!selectedReport) return ["pdf", "excel", "csv"]
    const report = reportTypes.find((r) => r.id === selectedReport)
    return report ? report.format.map((f) => f.toLowerCase()) : ["pdf", "excel", "csv"]
  }

  // Reset export format when report changes
  const handleReportSelect = (reportId: string) => {
    setSelectedReport(reportId)
    const report = reportTypes.find((r) => r.id === reportId)
    if (report && report.format.length > 0) {
      setExportFormat(report.format[0].toLowerCase())
    }
  }
